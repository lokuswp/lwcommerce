...
class AJAX {
	...
	public function order_action() {
		...
		if ( $action === 'pending' ) {
			$license = $this->generate_license_key( $order_id );
	...
	private function generate_license_key( $order_id ) {
		$la_url    = 'https://auth.lokuswp.id';
		$la_key    = 'JSBotJmjI4ofW4I2gBkdqtpPalfrcazZhR5tUwqbKu0qKv8kWPY5JeZV';
		$la_app_id = '2840125e-c04d-4052-9790-b70828ba9a9a';

		$di_url = 'https://dev.digitalcraft.id';

		$user_email = lwc_get_order_meta( $order_id, '_billing_email' );
		$user_name  = lwc_get_order_meta( $order_id, '_billing_name' );
		$user_phone = lwc_get_order_meta( $order_id, '_billing_phone' );
		...
		$user     = $this->get_user( $auth_data, $user_data );
		$products = $this->get_products_by_order_id( $order_id );

		// check is user exist in lokus auth
		if ( empty( $user['user'] ) ) {
			$user = $this->create_user( $auth_data, $user_data );
		}

		$user_id = $user['user']['id'];

		$licenses_status = [];
		// generate license for products
		foreach ( $products as $product ) {
			$body = [
				'slug'       => $product->digitalcraft_slug,
				'user_email' => $user_email,
				'user_uuid'  => $user_id,
				'order_id'   => (int) $order_id,
				'package'    => get_post_meta( $product->post_id, '_variation_name', true ),
			];

			$body = wp_json_encode( $body );

			$response = wp_remote_post( $di_url . '/api/v1/license', [
				'headers' => [
					'Content-Type'  => 'application/json',
					'Authorization' => 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImNhZTU2M2I3YyJ9..QqMaqSKHW2zuvIg2TvqlR-6QUayO-FDMHMUUzv-OvAo',
				],
				'body'    => $body,
				'timeout' => 30,
			] );
			...
			$licenses_status[] = $license['success'];
		}
		
		lwc_update_order_meta( $order_id, '_order_status', 'shipped' );

		return ! in_array( false, $licenses_status, true );
	}

	private function get_products_by_order_id( $order_id ) {
		global $wpdb;
		...
		$sql = "SELECT t.transaction_id, t.cart_uuid, c.cart_uuid, c.post_id, p.post_name ,
				MAX(CASE WHEN pm.meta_key = '_product_digitalcraft_slug' THEN pm.meta_value ELSE 0 END) digitalcraft_slug
				FROM $table_transactions AS t
				INNER JOIN $table_carts AS c ON t.cart_uuid = c.cart_uuid
				INNER JOIN $table_posts AS p ON c.post_id = p.ID
				LEFT JOIN $table_postmeta AS pm ON c.post_id=pm.post_id
				WHERE t.transaction_id = $order_id";

		return $wpdb->get_results( $sql );
	}

	private function get_user( $auth_data, $user_data ) {
		$url = $auth_data['la_url'] . '/api/user?email=' . $user_data['email'];

		$response      = wp_remote_get( $url, [
			'headers' => [
				'Authorization' => $auth_data['la_key'],
			],
		] );
	...
	private function create_user( $auth_data, $user_data ) {
		$url = $auth_data['la_url'] . '/api/user/registration';

		$body = [
			'registration' => [
				'applicationId'  => $auth_data['la_app_id'],
				'data'           => [
					'displayName' => $user_data['name'],
				],
				'roles'          => [ 'buyer' ],
				'usernameStatus' => 'ACTIVE',
				'username'       => strstr( $user_data['email'], '@', true )
			],
			'user'         => [
