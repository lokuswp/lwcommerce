...
function lwc_post_transaction_field( $trx_uuid ) {
    ...
    {{#shipping_name}}
    <div class="row mb-2" style="margin-top:12px;">
        <div class="col-xs-6"><?php _e( "Shipping Method", "lwcommerce" ); ?></div>
        <div id="lwc-order-id" class="col-xs-6 txt-right"><strong>{{shipping_name}}</strong></div>
    </div>
    {{/shipping_name}}

    {{#shipping_status_text}}
    <div class="row mb-2">
        <div class="col-xs-6"><?php _e( "Shipping Status", "lwcommerce" ); ?></div>
        <div id="lwc-order-status" class="col-xs-6 txt-right">{{shipping_status_text}}</div>
    </div>
    {{/shipping_status_text}}
	<?php
}
...
function lwc_transaction_response( $response, $trx_id ) {
	...
	$response['nav_history_url'] = get_permalink( lwp_get_ID_by_shortcode( "lokuswp_history" ) );

	$response['btn_text'] = __( "Payment Confirmation", "lwcommerce" );
...
//	$response['shipping_name']        = "Take Away";
//	$response['shipping_status_text'] = "Dalam Proses";
