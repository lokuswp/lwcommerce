...
module.exports = function createTests() {
    test('First install wordpress php 7.3', async ({page, browser}) => {
        await page.goto('http://localhost:8001/wp-admin/install.php');
        ...
        await expect(page).toHaveURL('http://localhost:8001/wp-admin/install.php?step=1');
        ...
        await expect(page).toHaveURL('http://localhost:8001/wp-admin/install.php?step=2');
        await page.locator('text=Log In').click();
        await expect(page).toHaveURL('http://localhost:8001/wp-login.php');
